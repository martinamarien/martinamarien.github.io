(function($, window, document, undefined) {
  var pluginName = 'ik_slider',
    defaults = {
      instructions:
        'use the right and left arrow keys to increase or decrease the slider value.',
      minValue: 0,
      maxValue: 100,
      nowValue: 0,
      step: 1
  ...
  Plugin.prototype.init = function() {
    ...
    if (!plugin.textfield.is(':text')) {
      throw plugin._name +
        ' plugin must be used only with text input elements.';
    } else {
      plugin.textfield
        .attr({
          readonly: '',
      ...
      plugin.element = plugin.textfield
        .parent('div')
        .addClass('ik_slider')
      ...
      plugin.fill = $('<div/>').addClass('ik_fill');
      ...
      plugin.knob = $('<div/>')
        ...
        .on('mouseleave', function() {
          setTimeout(plugin.onMouseUp, 100, { data: { plugin: plugin } });
        });
    ...
    if (event.data.plugin.dragging) {
      ...
      value =
        percent <= 0
          ...
          : percent >= 1
            ? max
            : min + Math.floor((max - min) * percent);
      mod = (value - min) % step;
  ...
  Plugin.prototype.onKeyDown = function(event) {
    ...
    switch (event.keyCode) {
      case ik_utils.keys.right:
        ...
        value =
          value < plugin.options.maxValue ? value : plugin.options.maxValue;
      ...
      case ik_utils.keys.left:
        value = parseInt($elem.attr('aria-valuenow')) - plugin.options.step;
        value =
          value > plugin.options.minValue ? value : plugin.options.minValue;
        plugin.setValue(value);
        break;
