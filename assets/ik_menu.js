(function($, window, document, undefined) {
  var pluginName = 'ik_menu',
    defaults = {
      instructions:
        'Use arrow keys to navigate between menuitems, spacebar to expand submenus, escape key to close submenus, enter to activate menuitems.'
    };
  ...
  Plugin.prototype.hideSubmenu = function(event) {
    ...
    if ($submenu.length) {
      $elem.removeClass('expanded').attr({
        'aria-expanded': false
      });

      $submenu.attr({ 'aria-hidden': true });
      $submenu.children({ tabindex: -1 });
    }
  };
  ...
  Plugin.prototype.onKeyDown = function(event) {
    ...
    switch (event.keyCode) {
      ...
      case ik_utils.keys.left:
        event.preventDefault();
        // if ($parentitem.parent('ul')[0] == $elem.children()[0]) {
        //   $parentitem.removeClass('expanded').attr({
        //     tabindex: 0,
        //     'aria-expanded': false
        //   });
        //   plugin.collapseAll(plugin, $parentitem);
        //   $parentitem.attr({ tabindex: -1 });

        //   var sibling = $parentitem.prev('li');
        //   console.log(sibling);
        //   sibling.attr({ tabindex: 0 });

        //   var siblingSubmenu = $parentitem.prev('li').children('ul');

        //   if (siblingSubmenu.length) {
        //     sibling.addClass('expanded').attr({
        //       'aria-expanded': true,
        //       tabindex: -1
        //     });
      ...
      case ik_utils.keys.down:
        event.preventDefault();
        event.stopPropagation();
        if ($submenu.length && $parentmenu[0] == $elem.children()[0]) {
